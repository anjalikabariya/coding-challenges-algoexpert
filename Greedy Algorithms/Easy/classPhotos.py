...
that you'll be photographing has an even number of students, and all these students are wearing red or
blue shirts. In fact, exactly half of the class is wearing red shirts, and the other half is wearing blue shirts.
You're responsible for arranging the students in two rows before taking the photo. Each row should
contain the same number of the students and should adhere to the following guidelines:
...
def classPhotos(redShirtHeights, blueShirtHeights):
    ...
    if ((tall[0] >= short[0]) and (tall[len(tall)-1] <= short[len(short)-1])) or ((tall[0] <= short[0]) and (tall[len(tall)-1] >= short[len(short)-1])):
        return False
    for t,s in zip(tall, short):
        if t < s:
            return False
    return True
