...
# The root directory is represented by a / . This means that if a path starts with / , it's an absolute
# path; if it doesn't, it's a relative path.
# The symbol / otherwise represents the directory separator. This means that the path /foo/bar
# is the location of the directory bar inside the directory foo , which is itself located inside the root
# directory.
...
# cannot, however, because repeating it sequentially means going further up in parent directories. For
# example, /foo/bar/baz/././. and /foo/bar/baz are equivalent paths, but
# /foo/bar/baz/../../../ and /foo/bar/baz definitely aren't. The only exception is with the
# root directory: /../../.. and / are equivalent, because the root directory has no parent
