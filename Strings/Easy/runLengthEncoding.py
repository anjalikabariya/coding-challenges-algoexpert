...
stored as a single data value and count, rather than as the original run." For this problem, a run of data is
any sequence of consecutive, identical characters. So the run "AAA" would be run-length-encoded as
"3A" .
...
"12A" , since this string can be decoded as either "AAAAAAAAAAAA" or "1AA" . Thus, long runs (runs
of 10 or more characters) should be encoded in a split fashion; the aforementioned run should be
encoded as "9A3A" .
Sample Input
...
def runLengthEncoding(string):
    ...
    for i in range(1,len(string)):
        if string[i] == string[i-1] and count < 9:
            ...
            count = 1
    encoded += str(count) + string[len(string)-1]
    return encoded